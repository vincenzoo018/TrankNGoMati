...
namespace TrackNGoMati.Areas.Admin.Controllers
{
    ...
    public class ReportsController : Controller
    {
        ...
        {
            ViewData["Title"] = "System Analytics & Reports";
            ViewBag.CurrentPage = "Reports";

            // Analytics Data
            var allDocs = _context.Documents.ToList();
            
            // SLA Compliance Pie Chart
            var completedDocs = allDocs.Where(d => d.CurrentStatus == 3).ToList();
            var breached = completedDocs.Count(d => (d.DateCompleted ?? DateTime.Now).Subtract(d.DateFiled).TotalDays > d.ArtaprocessingDays);
            var withinLimit = completedDocs.Count - breached;
            
            ViewBag.SlaBreached = breached;
            ViewBag.SlaWithinLimit = withinLimit;

            // Volume by Status
            ViewBag.StatusPending = allDocs.Count(d => d.CurrentStatus == 1);
            ViewBag.StatusInProgress = allDocs.Count(d => d.CurrentStatus == 2);
            ViewBag.StatusCompleted = completedDocs.Count;

            // Average Processing Time per Department (mocked/calculated from completed docs)
            var depts = _context.Departments.ToList();
            var processingTimes = new List<object>();
            foreach(var dept in depts)
            {
                var deptDocs = completedDocs.Where(d => d.OriginatingDepartment == dept.DepartmentName).ToList();
                double avg = deptDocs.Any() ? deptDocs.Average(d => (d.DateCompleted ?? DateTime.Now).Subtract(d.DateFiled).TotalDays) : 0;
                processingTimes.Add(new { label = dept.DepartmentCode, value = Math.Round(avg, 1) });
            }
            ViewBag.ProcessingTimes = processingTimes;
        ...
        public IActionResult ExportCsv(string exportType, string exportPassword)
        {
            ...
            var pwHash = Convert.ToHexString(System.Security.Cryptography.SHA256.HashData(System.Text.Encoding.UTF8.GetBytes(exportPassword))).ToLower();
